...
| `web/levels.js` | the C34 level-campaign runtime — the registry level files register into (`PIO_LEVEL`), the versioned square-wave monitor judge (the profile ladder's tier parameters; `{defect:'judge'}` re-injects the single-glitch pass), the predict gate (`{defect:'gate'}` re-injects never-locks), the level→stored-program builder (canonical listing + overlay through `PioAsm`/`VibeDriver.parseState`), the SURFACE table (panel gating = absence), and the C42 campaign data: the `CHAPTERS` title table (validate() rejects a level whose chapter has no title), `sessionKey` (the per-level localStorage spelling both pages share), `campaign` (order/solved/frontier/chapters over the existing keys — pure, the levels gate pins it), and `parText` (the named-clock par label the band and the map both show) |
...
| `web/index.html` | the C42 campaign landing — the front door at `/` (web/serve.py direct-serves it; the Pages artifact redirects to it): its own page sharing `sm-view.css`, classic-script dependency-free, NO engine boot. Inline glue only (the C25 listbox grammar + the era tooltip); the campaign derivation is `levels.js` module logic. The sandbox stays at `web/sm-view.html` for tool users |
