...
Pipeline for one case (--a/--b, words or .pio programs; --horizon N):
  ...
  1. Python-model pre-filter (C12): both programs run in lockstep through
     ...
     miter's fixed config (EXECCTRL per --execctrl, per-side under the
     C14 overlay SPEC-16-8; CLKDIV/SHIFTCTRL/PINCTRL at reset — the v1
     single-config scope, SPEC-16-6). A SPEC-16-7 trace divergence is a
     definite FAIL, decoded on the spot.
...
class ProgPair:
    """One equivalence case: two imem images + the SM0 config the C11
    miter expresses (EXECCTRL only, per-side under the C14 overlay,
    SPEC-16-8; everything else resets, SPEC-16-6).

    execctrl_b=None means a shared EXECCTRL (the v1 case); the overlay
    form sets it, and the pair's intentionally-differing bits are then
    masked out of EXECCTRL readback compares (pio_equiv_miter's
    EC_CMP_MASK)."""

    words_a: tuple[int, ...]
    words_b: tuple[int, ...]
    execctrl: int
    execctrl_b: int | None = None

    @property
    def ec_b(self) -> int:
        """Side B's EXECCTRL (side A's when not overlaid)."""
        return self.execctrl if self.execctrl_b is None else self.execctrl_b

    @property
    def ec_mask(self) -> int:
        """EXECCTRL readback-compare mask (SPEC-16-8): 0x7fffffff minus
        the pair's intentional overlay difference.

        >>> hex(ProgPair((0,), (0,), 0x4000).ec_mask)
        '0x7fffffff'
        >>> hex(ProgPair((0,), (0,), 0x4000, 0x5000).ec_mask)  # wrap_top overlaid
        '0x7fffefff'
        """
        return 0x7FFFFFFF & ~((self.execctrl ^ self.ec_b) & 0x7FFFFFFF)
...
def wrapper_sv(pair: ProgPair) -> str:
    ...
    SM0_EXECCTRL_A/B literal overrides on pio_equiv_miter (the committed
    pio_equiv_miter_red pattern, SPEC-16-4/6/8)."""
...
// program pair, prologue length and per-side SM0_EXECCTRL (SPEC-16-4/6,
// overlay per SPEC-16-8).
module {WRAPPER_MOD} (
    input  logic        clk,
    input  logic        rst,
  ...
  pio_equiv_miter #(
      ...
      .SM0_EXECCTRL_A(32'h{pair.execctrl:08X}),
      .SM0_EXECCTRL_B(32'h{pair.ec_b:08X})
  ) u_m (
      .clk (clk), .rst (rst),
      .gpio_in (gpio_in),
...
def disasm_word(pair: ProgPair, side: str, pc: int) -> str:
    ...
    (sideset count 0 at PINCTRL reset, side_en from that side's
    EXECCTRL — per-side under the SPEC-16-8 overlay)."""
    w = pair.word(side, pc & 0x1F)
    ec = pair.execctrl if side == "a" else pair.ec_b
    try:
        return disasm.disassemble(w, ((ec >> 30) & 1) != 0, 0)
...
def run_pair_models(
    ...
    """Both programs in lockstep (prologue per SPEC-16-4 — EXECCTRL
    per-side under the SPEC-16-8 overlay — shared free stimulus per
    SPEC-16-5) -> (recs_a, recs_b, per-clk entry PCs)."""
    ...
    recs_b += _step_rec(mb, clk, OP_WR, stim.A_SM0 + 4 * 1, pair.ec_b, IDLE)  # SPEC-16-8
    clk += 1
    pcs.append((ma.pc_r, mb.pc_r))
...
def decode_first_divergence(
    recs_a: Sequence[tracefmt.Rec],
    recs_b: Sequence[tracefmt.Rec],
    pcs: Sequence[tuple[int, int]],
    src: str,
    ec_mask: int = 0x7FFFFFFF,
) -> Divergence | None:
    """First SPEC-16-7 trace divergence -> Divergence (None if equal).

    ec_mask carries the pair's EXECCTRL overlay exclusions (SPEC-16-8).

    >>> decode_first_divergence([("G", 0, 1, 0, 0)], [("G", 0, 1, 0, 0)], [(0, 0)], "")
    >>> decode_first_divergence([("G", 0, 5, 0, 0)], [("G", 0, 4, 0, 0)], [(1, 2)], "x").pin
    0
    """
    a, b = tracefmt.normalize(recs_a, ec_mask), tracefmt.normalize(recs_b, ec_mask)
...
def prefilter(pair: ProgPair, horizon: int, rounds: int, seed: int) -> Divergence | None:
    ...
    for r in range(rounds):
        ...
        div = decode_first_divergence(
            recs_a, recs_b, pcs, f"pre-filter round {r + 1}/{rounds}, seed {seed}", pair.ec_mask
        )
        if div is not None:
            return div
    return None
...
def decode_vcd(samples: Sequence[Sample], ec_mask: int = 0x7FFFFFFF) -> Divergence | None:
    """First VCD observable divergence (SPEC-16-1 per clk, SPEC-16-2 at
    read strobes with the pair's overlay mask, SPEC-16-8 — both taken
    from the twin's own signals)."""
    recs_a, recs_b, pcs = samples_to_recs(samples)
    return decode_first_divergence(recs_a, recs_b, pcs, "sby counterexample", ec_mask)
...
def _read_subst(addr: int) -> int:
    ...
    True
    >>> _read_subst(stim.A_FSTAT) == stim.A_FSTAT
    True
    """
    widx = (addr >> 2) & 0x7F
    sm_sel = ((widx - 50) // 6) if 50 <= widx <= 73 else 0
    if (50 <= widx <= 73 and sm_sel != 0) or widx in (9, 10, 11):  # RXF1..3
        return stim.A_FSTAT
    return addr


def replay_cex(samples: Sequence[Sample], ec_mask: int = 0x7FFFFFFF) -> tuple[Divergence | None, str]:
    ...
    alike), environment from the shared free inputs. Out-of-scope CEX
    reads are substituted identically on both sides (see _read_subst).
    -> (divergence, note); a clean replay of a FAILing CEX is reported
    as a mismatch."""
    ma, mb = M.PIOBlockModel(), M.PIOBlockModel()
    recs_a: list[tracefmt.Rec] = []
    recs_b: list[tracefmt.Rec] = []
    ...
    for s in samples:
        ...
        addr = _read_subst(v.get("addr", 0)) if op == OP_RD else v.get("addr", 0)
        pcs.append((ma.pc_r, mb.pc_r))
        oa = ma.step(
            v.get("gpio_in", 0),
            0,
        ...
        if op == OP_RD:
            ...
            recs_a.append(("R", clk, addr, oa["rdata"]))
            recs_b.append(("R", clk, addr, ob["rdata"]))
        clk += 1
    div = decode_first_divergence(recs_a, recs_b, pcs, "CEX replay in golden model", ec_mask)
    if div is None:
...
def check_pair(
    ...
    *,
    ...
    if pair.execctrl_b is None:
        head.append(
            f"SM0_EXECCTRL 0x{pair.execctrl:08x} (wrap {(pair.execctrl >> 12) & 0x1F}->{(pair.execctrl >> 7) & 0x1F}); "
            f"horizon {horizon} clk (sby bmc depth)"
        )
    else:
        ea, eb = pair.execctrl, pair.ec_b
        head.append(
            f"SM0_EXECCTRL A 0x{ea:08x} (wrap {(ea >> 12) & 0x1F}->{(ea >> 7) & 0x1F}) / "
            f"B 0x{eb:08x} (wrap {(eb >> 12) & 0x1F}->{(eb >> 7) & 0x1F}) — overlay (SPEC-16-8); "
            f"horizon {horizon} clk (sby bmc depth)"
        )
    if prefilter_on:
    ...
    if res["status"] == "FAIL":
        ...
        div = decode_vcd(samples, pair.ec_mask)
        lines = [*head, "-- sby bmc: FAIL (counterexample)"]
        if div is not None:
            lines += div.lines(pair)
        rdiv, note = replay_cex(samples, pair.ec_mask)
...
def _self_test_hermetic() -> list[tuple[str, bool]]:
    ...
    checks.append(
        (
            "wrapper-params",
            "16'h80A0" in w and "PROG_LEN(5'd5)" in w and "SM0_EXECCTRL_A(32'h00004000)" in w,
        )
    )
    wo = wrapper_sv(ProgPair(GREEN_A, GREEN_B, 0x4000, 0x5000))
    checks.append(("wrapper-params-overlay", "SM0_EXECCTRL_B(32'h00005000)" in wo))
    ...
    # EXECCTRL overlay (SPEC-16-8): a dead-field difference (STATUS_N,
    # only observable via MOV status — absent here) stays green; a
    # wrap-field difference that changes flow goes red.
    checks.append(("prefilter-overlay-green", prefilter(ProgPair(GREEN_A, GREEN_B, 0x4000, 0x401F), 24, 4, 1) is None))
    checks.append(
        ("prefilter-overlay-red", prefilter(ProgPair(GREEN_A, GREEN_B, 0x4000, 0x4080), 24, 4, 1) is not None)
    )
    ...
    # CEX replay must survive out-of-scope read addresses (SPEC-16-4):
    # the C14 tamper e2e's counterexample sampled the anyconst read at
    # 0x104 (SM2 EXECCTRL) and the replay crashed before _read_subst.
    sm2_samples = [
        Sample(t=0, step=0, rst=1, vals={}),
        Sample(t=10, step=1, rst=0, vals={"read": 1, "addr": 0x104, "rd_a": 5, "rd_b": 5}),
    ]
    rdiv, rnote = replay_cex(sm2_samples)
    checks.append(("replay-sm2-read-subst", rdiv is None and "no divergence" in rnote))
    return checks
