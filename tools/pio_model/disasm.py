...
_JMP_CONDS: dict[int, str] = {v: k for k, v in E.JMP_CONDS.items() if k is not None}
_IN_SRCS: dict[int, str] = {v: k for k, v in E.IN_SRCS.items()}
_OUT_DSTS: dict[int, str] = {v: k for k, v in E.OUT_DSTS.items()}
_MOVD_DSTS: dict[int, str] = {v: k for k, v in E.MOVD_DSTS.items() if k is not None and v is not None}
_MOVS_SRCS: dict[int, str] = {v: k for k, v in E.MOVS_SRCS.items() if k is not None and v is not None}
_MOVS_SRCS[E.MOVS_STATUS] = "status"
_SET_DSTS: dict[int, str] = {v: k for k, v in E.SET_DSTS.items()}
_IDX_MODES: dict[int, str] = {v: k for k, v in E.IDX_MODES.items() if k}
_OPS: dict[int, str] = {E.MOP_NONE: "", E.MOP_INV: "~", E.MOP_REV: "::"}


def _postfix(d: E.Decoded) -> str:
    s = ""
    if d["ss_valid"]:
        s += f" side {d['ss_val']}"
    if d["delay"]:
...
def disassemble(word: int, side_en: bool = False, sideset_count: int = 0) -> str:
    ...
    if d["illegal"]:
        raise E.ReservedEncoding(f"reserved encoding {word & 0xFFFF:04x} (SPEC-13-1)")
    ...
    repack = E.pack_ds(d["delay"], d["ss_val"], side_en, sideset_count, d["ss_valid"])
    if repack != ((word >> 8) & 0x1F):
        raise E.ReservedEncoding(f"aliased delay/side-set field {(word >> 8) & 0x1F:02x} (SPEC-4-1..3)")
    pf = _postfix(d)

    def count(n: int) -> int:  # SPEC-2-18: bitcount 0 => 32
        return n or 32
    ...
    if d["is_wait"]:  # SPEC-3.2
        if d["wait_src"] == E.WSRC_IRQ:
            ...
            return f"wait {d['wait_pol']} irq {d['wait_index'] & 7}" + (f" {mode}" if mode else "") + pf
    ...
    if d["is_push"] or d["is_pull"]:  # SPEC-3.5
        ...
        if d["is_pull"] and d["pull_ife"]:
            m += " ifempty"
        m += " noblock" if not (d["push_blk"] if d["is_push"] else d["pull_blk"]) else " block"
    ...
    if d["is_put"] or d["is_get"]:  # SPEC-3.7
        ...
        if a1 != (4 if d["is_get"] else 0):
            raise E.ReservedEncoding(f"fifo-aux arg1 {a1} reserved (SPEC-2-12)")
    ...
    if d["is_mov"]:  # SPEC-3.6
        ...
        return f"mov {_MOVD_DSTS[d['mov_dst']]}, {_OPS[d['mov_op']]}{_MOVS_SRCS[d['mov_src']]}{pf}"
