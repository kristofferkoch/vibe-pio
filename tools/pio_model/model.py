...
class PIOBlockModel:
    ...
    def reset(self):
        ...
        self.ctrl_r = 0                        # CTRL.SM_ENABLE (SPEC-7-2)
        self.isb_r = 0                         # INPUT_SYNC_BYPASS (SPEC-7-7)
        self.imem = [0] * 32                   # pio_instr_mem all-zero reset
        # pio_irq_flags.sv
        self.flags_r = 0
        # pio_gpio_mux.sv
        self.sync1_r = 0
        ...
        self.gpio_lvl_r = 0
        self.gpio_oe_r = 0
        self.sticky_mask = [0] * 4             # SM0's record is live
        self.sticky_lvl = [0] * 4
        self.sticky_dir = [0] * 4
        self.sticky_is_dir = [0] * 4
        # pio_sm_regs.sv config banks (datasheet defaults)
        self.clkdiv_r = CLKDIV_RESET
        self.execctrl_r = EXECCTRL_RESET
        self.shiftctrl_r = SHIFTCTRL_RESET
        self.pinctrl_r = PINCTRL_RESET
        ...
        self.y_r = 0
        self.delay_cnt_r = 0
        self.latch_r = 0                       # CC-34/35 EXEC/force latch
        self.latch_vld_r = 0
        ...
        self.tx_level = 0
        self.rx_level = 0
        self.fdbg_tx_stall = 0                 # SPEC-6-7 stickies
        self.fdbg_rx_stall = 0
        self.fdbg_tx_over = 0
        self.fdbg_rx_under = 0
        self._mut_stall_seen = False           # delay_in_stall demo state
        return self
    ...
    @property
    def clkdiv_frac(self):
        return (self.clkdiv_r >> 8) & 0xFF

    @property
    def side_en(self):
        return _bit(self.execctrl_r, 30)       # SPEC-7-16

    @property
    def side_pindir(self):
        return _bit(self.execctrl_r, 29)
    ...
    @property
    def out_sticky_en(self):
        return _bit(self.execctrl_r, 17)       # SPEC-7-18
    ...
    @property
    def fjoin_rx(self):
        return _bit(self.shiftctrl_r, 31)      # SPEC-7-21

    @property
    def fjoin_tx(self):
        return _bit(self.shiftctrl_r, 30)

    @property
    def pull_thresh(self):
        return (self.shiftctrl_r >> 25) & 0x1F  # SPEC-5-7 (0 encodes 32)

    @property
    def push_thresh(self):
        return (self.shiftctrl_r >> 20) & 0x1F

    @property
    def out_shift_left(self):
        return not _bit(self.shiftctrl_r, 19)  # SPEC-5-2: 1 = right

    @property
    def in_shift_left(self):
        return not _bit(self.shiftctrl_r, 18)

    @property
    def autopull(self):
        return _bit(self.shiftctrl_r, 17)      # SPEC-5-8

    @property
    def autopush(self):
        return _bit(self.shiftctrl_r, 16)      # SPEC-5-9

    @property
    def fjoin_rx_put(self):
        return _bit(self.shiftctrl_r, 15)      # SPEC-6-3

    @property
    def fjoin_rx_get(self):
        return _bit(self.shiftctrl_r, 14)

    @property
    def in_mask_count(self):
    ...
    @property
    def fifo_mode(self):                       # pio_sm.sv decode (SPEC-6-2/3)
    ...
    def _flag_rd(self, flags, mode, idx):
    ...
    # ==================================================================
    def step(self, gpio_in, lb_mask=0, op=0, addr=0, wdata=0,
             nb_set=0, nb_clr=0, irq_prev=0, irq_next=0):
        ...
        if sm_hit and sm_sel != 0 and (reg_write or reg_read):
            raise ValueError(
                f"SM{sm_sel} window access 0x{addr:03x} is out of the "
                "single-SM scope (SPEC-16-4)")
        ...
        sys_aux_rd = reg_read and aux_hit and aux_sel == 0
        fdbg_clr = {                          # FDEBUG W1C (SPEC-7-29)
        ...
        if "sync_1ff" in self.mut:
            ...
            gpio_seen = 0
            for p in range(32):
                seen = _bit(self.sync1_r, p) if _bit(self.isb_r, p) \
                    else _bit(self.sync2_r, p)
                gpio_seen |= seen << p
        ...
        target = int_eff + (1 if (self.stretch_r and self.clkdiv_int != 0)
                            else 0)
        ...
        m_delay = tick_sm and not self.restart_pend_r \
            and self.state_r == ST_DELAY
        m_exec = tick_forced or (tick_sm and not self.restart_pend_r
                                 and self.state_r != ST_DELAY)

        # ---------------- instruction selection + decode (CC-33/34) ---
        src_latch = tick_forced or (self.latch_vld_r
                                    and not self.latch_force_r)
        ...
        first_tick = m_exec and (not self.forced_stall_r if tick_forced
                                 else self.state_r != ST_STALL)
        ...
        irq_set_req = first_tick and not d["illegal"] and d["is_irq"] \
            and not d.get("irq_clr", 0)
        wsrc = d.get("wait_src", 0)
        widx5 = d.get("wait_index", 0)
        irq_flag_idx = d.get("irq_index", 0) if d["is_irq"] else widx5 & 7
        irq_idx_mode = d.get("irq_idxmode", 0) if d["is_irq"] \
            else (widx5 >> 3) & 3
        ...
        flags_now = ((self.flags_r | loc_set_early) & ~loc_clr_early) & 0xFF \
            if "irq_same_cycle" in self.mut else self.flags_r
        ...
        if self.status_sel == 0:              # SPEC-3.6-12 TXLEVEL
            ...
            n = self.status_n
            mode = (((n >> 4) & 1) << 1) | ((n >> 3) & 1)
            status_val = M32 if self._flag_rd(flags_now, mode, n & 7) \
                else 0
        else:
        ...
        if wsrc == E.WSRC_GPIO:
            ...
            wait_pin = self._flag_rd(flags_now, (widx5 >> 3) & 3,
                                     widx5 & 7)
        ...
        mop = d.get("mov_op", 0)
        mov_result = (~mov_src_val & M32) if mop == E.MOP_INV \
            else _bitrev32(mov_src_val) if mop == E.MOP_REV else mov_src_val

        # Stall evaluation (SPEC-9, CC-11..CC-21)
        push_guard = (not d.get("push_iff", 0)) \
            or self.isr_cnt_r >= push_thr     # SPEC-3.5-5, CC-31
        pull_guard = (not d.get("pull_ife", 0)) \
            or self.osr_cnt_r >= pull_thr     # SPEC-3.5-10, CC-31
        pull_fence = (not self.autopull) or self.osr_cnt_r >= pull_thr
        irqw_flag = self._flag_rd(flags_now, d.get("irq_idxmode", 0),
                                  d.get("irq_index", 0))
        ...
        out_data = (self.osr_r >> (32 - out_count)) if self.out_shift_left \
            else (self.osr_r & out_mask)      # SPEC-3.4-1
        osr_shifted = (self.osr_r << out_count) & M32 \
            if self.out_shift_left else self.osr_r >> out_count
        ...
        if in_count == 32:
            ...
            isr_shifted = ((self.isr_r << in_count) & M32) \
                | (in_data & in_mask)
            isr_rotated = ((self.isr_r << in_count) & M32) \
                | (self.isr_r >> (32 - in_count))
        else:
            isr_shifted = ((self.isr_r >> in_count)
                           | ((in_data & in_mask) << (32 - in_count))) & M32
            isr_rotated = ((self.isr_r >> in_count)
                           | ((self.isr_r << (32 - in_count)) & M32)) & M32
        ...
        out_ap_ge = m_exec and d["is_out"] and not d["illegal"] \
            and ap_on and autopull_ge_thr     # CC-11

        stall = False
        if m_exec and not d["illegal"]:
            if d["is_wait"]:
                stall = not wait_cond         # CC-15
            elif d["is_irq"] and d.get("irq_wait", 0) \
                    and not d.get("irq_clr", 0):
                stall = not irqw_rel          # CC-16
            elif d["is_push"]:
                stall = push_guard and d.get("push_blk", 1) and rx_full \
                    and rx_queue              # CC-19
            elif d["is_pull"]:
                stall = pull_guard and pull_fence and d.get("pull_blk", 1) \
                    and tx_empty              # CC-20
            elif d["is_out"]:
                stall = out_ap_ge
        ...
        pc_wr_explicit = compl and not d["illegal"] and (
            (d["is_jmp"] and jmp_taken)
            or (d["is_out"] and d.get("out_dst", 0) == E.OUTD_PC)
            or (d["is_mov"] and d.get("mov_dst", 0) == E.MOVD_PC))
        pc_wr_val = d.get("jmp_addr", 0) if d["is_jmp"] \
            else (out_data & 0x1F if d["is_out"] else mov_result & 0x1F)
        if pc_wr_explicit:
            pc_next = pc_wr_val
        elif src_latch:
        ...
        just_latched = compl and not d["illegal"] and (
        ...
        x_wr = compl and not d["illegal"] and (
        ...
        y_wr = compl and not d["illegal"] and (
        ...
        ap_post_refill = compl and d["is_out"] and not d["illegal"] \
            and ap_on and autopull_post_thr and not tx_empty
        ap_bg = m_exec and not d["illegal"] and not d["is_out"] and ap_on \
            and autopull_ge_thr and not tx_empty \
            and not (d["is_pull"] or (d["is_get"] and get_ok)
                     or (d["is_mov"] and d.get("mov_dst", 0) == E.MOVD_OSR))
        pull_load = compl and not d["illegal"] and d["is_pull"] \
            and pull_guard and pull_fence and not tx_empty   # CC-20
        pull_fallb = compl and not d["illegal"] and d["is_pull"] \
            and pull_guard and pull_fence and not d.get("pull_blk", 1) \
            and tx_empty                       # CC-32, SPEC-14.7-1
        get_en = compl and not d["illegal"] and d["is_get"] and get_ok
        mov_osr = compl and not d["illegal"] and d["is_mov"] \
            and d.get("mov_dst", 0) == E.MOVD_OSR

        aux_idx = d.get("aux_index", 0) if d.get("aux_idxi", 0) \
            else self.y_r & 3                 # SPEC-3.7-4
        ...
        osr_wr_en = m_restart or ap_ge_refill or ap_post_refill or ap_bg \
            or pull_load or pull_fallb or get_en or mov_osr
        if m_restart:
        ...
        in_ap = compl and not d["illegal"] and d["is_in"] and aph_on \
            and autopush_req and not rx_full
        push_do = compl and not d["illegal"] and d["is_push"] \
            and rx_queue and push_guard
        rx_push = in_ap or (push_do and not rx_full)
        rx_push_data = self.isr_r if d["is_push"] else autopush_data

        out_isr = compl and not d["illegal"] and d["is_out"] \
            and d.get("out_dst", 0) == E.OUTD_ISR
        mov_isr = compl and not d["illegal"] and d["is_mov"] \
            and d.get("mov_dst", 0) == E.MOVD_ISR
        ...
        tx_stall_req = (m_exec and not d["illegal"] and d["is_pull"]
                        and pull_guard and pull_fence
                        and d.get("pull_blk", 1) and tx_empty) \
            or (out_ap_ge and tx_empty)
        rx_stall_req = (m_exec and not d["illegal"] and d["is_in"]
                        and aph_on and autopush_req and rx_full) \
            or (push_do and not d.get("push_blk", 1) and rx_full)
        ...
        gpio_out_we = compl and not d["illegal"] and (
            (d["is_out"] and d.get("out_dst", 0) in
             (E.OUTD_PINS, E.OUTD_PINDIRS))
            or (d["is_mov"] and d.get("mov_dst", 0) in
                (E.MOVD_PINS, E.MOVD_PINDIRS)))
        gpio_out_pindir = (d.get("out_dst", 0) == E.OUTD_PINDIRS
                           if d["is_out"]
                           else d.get("mov_dst", 0) == E.MOVD_PINDIRS)
        gpio_out_data = out_data if d["is_out"] else mov_result
        gpio_set_we = compl and not d["illegal"] and d["is_set"] and \
            d.get("set_dst", 0) in (E.SETD_PINS, E.SETD_PINDIRS)

        # IRQ clear request (SPEC-3.8-1; WAIT-1-irq completing clear
        # CC-15). The set request is computed early (flag view above).
        irq_clr_req = compl and not d["illegal"] and (
            (d["is_irq"] and d.get("irq_clr", 0))
            or (d["is_wait"] and wsrc == E.WSRC_IRQ
                and d.get("wait_pol", 1)))

        # ---------------- pio_gpio_mux output resolution (CC-6/CC-7) --
        def place_mask(base, n):
            if n == 0 or n > 31:
        ...
        if self.out_sticky_en and not self.sticky_is_dir[0]:  # CC-5
            lvl_next = ((lvl_next & ~self.sticky_mask[0])
                        | self.sticky_lvl[0]) & M32
        if self.out_sticky_en and self.sticky_is_dir[0]:
            oe_next = ((oe_next & ~self.sticky_mask[0])
                       | self.sticky_dir[0]) & M32
        ...
        sticky_isdir_next = gpio_out_pindir if gpio_out_we \
            else d.get("set_dst", 0) == E.SETD_PINDIRS
        sticky_mask_next = (os_lvl_mask | os_dir_mask) if sticky_wr \
            else self.sticky_mask[0]
        sticky_lvl_next = os_lvl_data if (sticky_wr and not sticky_isdir_next) \
            else self.sticky_lvl[0]
        sticky_dir_next = os_dir_data if (sticky_wr and sticky_isdir_next) \
            else self.sticky_dir[0]
        ...
        rx_queue_wr = tick_any and rx_push and rx_depth != 0 \
            and self.rx_level < rx_depth
        ...
        aux_put_en = tick_any and aux_put and \
            self.fifo_mode in (FM_TXPUT, FM_PUTGET)
        ...
        fstat = ((0x0F00_0F00 & ~sm0_mask)
                 | ((1 if tx_empty else 0) << 24)
                 | ((1 if tx_full else 0) << 16)
                 | ((1 if rx_empty else 0) << 8)
                 | ((1 if rx_full else 0) << 0))
        fdebug = ((self.fdbg_tx_stall << 24) | (self.fdbg_tx_over << 16)
                  | (self.fdbg_rx_under << 8) | self.fdbg_rx_stall)
        flevel = (self.tx_level & 0xF) | ((self.rx_level & 0xF) << 4)
        intr = ((flags_now << 8)
                | 0xE0                         # SM3..1 TXNFULL = 1
                | ((0 if tx_full else 1) << 4)
                | (0 if rx_empty else 1)) & 0xFFFF
        execctrl_rb = (self.execctrl_r & 0x7FFFFFFF) \
            | (0x80000000 if self.forced_stall_r else 0)  # SPEC-7-15
        ...
        if widx == 0:
            ...
            rdata = (self.clkdiv_r if sm_reg == 0
                     else execctrl_rb if sm_reg == 1
                     else self.shiftctrl_r if sm_reg == 2
                     else self.pc_r if sm_reg == 3       # SPEC-7-22
                     else instr_mem_w if sm_reg == 4     # SPEC-7-24
                     else self.pinctrl_r)
        elif aux_hit:
        ...
        obs = {"gpio_out": self.gpio_lvl_r, "gpio_oe": self.gpio_oe_r,
               "intr": intr, "rdata": rdata if reg_read else None,
               "read": reg_read, "addr": addr}
        ...
        if m_restart:
            ...
            if self.state_r == ST_DELAY or (self.state_r == ST_EXEC
                                            and self.latch_force_r):
                self.state_r = ST_FETCH
        ...
        if sm_restart:
            ...
            if m_restart:
                self.restart_pend_r = 0
            if m_exec and not d["illegal"] and d["is_irq"] \
                    and d.get("irq_wait", 0) and not d.get("irq_clr", 0):
        ...
        if flush:
            ...
            self.rx_level += (1 if rx_queue_wr else 0) \
                - (1 if rx_queue_rd else 0)
