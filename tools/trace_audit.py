...
def _strip_suffix(el):
    ...
    if m and not re.search(r"\d", m.group(2)) and VALID_BASE.fullmatch(m.group(1)):
        return m.group(1)
    return el
...
def expand_run(run):
    ...
    for raw in tok.split("/"):
        ...
        el = _strip_suffix(raw)
        if m := SPEC_RANGE.fullmatch(el):
            sec, lo, hi = m.group(1), int(m.group(2)), int(m.group(3))
            out.append(
                Range(f"SPEC-{sec}-{lo}", f"SPEC-{sec}-{hi}", [(f"SPEC-{sec}-{n}", False) for n in range(lo, hi + 1)])
            )
        elif m := CC_RANGE.fullmatch(el):
            lo, hi = int(m.group(1)), int(m.group(2))
            out.append(Range(f"CC-{lo}", f"CC-{hi}", [(f"CC-{n}", False) for n in range(lo, hi + 1)]))
...
class SourceFile:
    def __init__(self, path, repo=None):
        ...
        self.rel = str(self.path.relative_to(repo)) if self.path.is_relative_to(repo) else str(self.path)
    ...
    def line_bounds(self, idx):  # idx is 1-based
        ...
        end = self.line_starts[idx] - 1 if idx < len(self.line_starts) else len(self.text)
        return start, end
    ...
    def _build_registry(self):
        ...
        in_head = True
        seen_entry = False
        entry, entry_keys, entry_indent = None, [], -1  # indent: -1 = unset
        for line in self.lines:
            ...
            if entry is not None:
                ...
                if ind > entry_indent and not re.fullmatch(r"[A-Za-z]\w{0,3}\d\w{0,2}", first):
                    for key in entry_keys:
    ...
    for m in STMT_KW.finditer(sf.code):
        ...
        while i >= 0:
            if FULL_COMMENT.match(sf.lines[i]):
                ...
                i -= 1
                continue
            if structural_left and not STMT_KW.search(code_lines[i]) and STRUCTURAL.fullmatch(code_lines[i]):
                structural_left -= 1
                i -= 1
                continue
            break
        stmts.append({"kind": m.group(1), "label": label, "line": line, "owned": owned})
    return stmts
...
def registry_runs(sf, stmt):
    ...
    for tok in re.findall(r"[A-Za-z]\w{0,3}", " ".join(sf.text[s:e] for s, e in stmt["owned"])):
        names.add(tok.lower())
    out = []
    for key in sf.registry:
        ...
        for part in names:
            if k == part or (
                len(k) >= 2 and any(c.isdigit() for c in k) and part.startswith(k) and not part[len(k)].isdigit()
            ):
...
class Audit:
    def __init__(self, spec_path, cc_path, source_patterns, repo=None):
        self.repo = REPO if repo is None else Path(repo)
        self.defects = []
        self.fact_lines = {}
        self.clause_lines = {}
        self.cc_body = {}
        ...
        for pat in source_patterns:
            paths.extend(sorted(Path(q) for q in glob.glob(str(pat))))
        self.sources = [SourceFile(p, self.repo) for p in paths]
    ...
    def _parse_indexes(self, spec_path, cc_path):
        for i, line in enumerate(Path(spec_path).read_text().split("\n"), 1):
            for m in re.finditer(r"\[SPEC-[0-9.]+-[0-9]+\]", line):
                ...
                if fid in self.fact_lines:
                    self.defects.append(f"DUP-ID: {fid} defined at both {self.fact_lines[fid]} and {i} of {spec_path}")
                else:
        ...
        for cid in sorted(set(self.cc_body) - set(self.clause_lines), key=id_key):
            self.defects.append(
                f"INDEX-MISMATCH: {cid} in cycle-contract body (line {self.cc_body[cid]}) but not in the clause index"
            )
        for cid in sorted(set(self.clause_lines) - set(self.cc_body), key=id_key):
            self.defects.append(
                f"INDEX-MISMATCH: {cid} in cycle-contract index (line {self.clause_lines[cid]}) but not in the body"
            )
        ...
        for part in parts:
            if isinstance(part, Range):
                if not self._defined(part.lo) or not self._defined(part.hi):
                    self.defects.append(f"DANGLING: range {part.lo}..{part.hi} endpoint not defined at {rel}:{line}")
                    continue
                ids.extend((cid, wide) for cid, wide in part.ids if self._defined(cid))
            elif isinstance(part, SecRange):
                lo, hi = sec_tuple(part.lo), sec_tuple(part.hi)
                members = [f for f in self.fact_lines if lo <= sec_tuple(f[5:].rsplit("-", 1)[0]) <= hi]
                if members:
                    ids.extend((f, True) for f in members)
                else:
                    self.defects.append(
                        f"DANGLING: section range SPEC-{part.lo}..{part.hi} has no defined facts at {rel}:{line}"
                    )
            else:
                cid, wide = part
                if wide and not self._defined(cid):
                    ...
                    members = [f for f in self.fact_lines if f.startswith(cid + "-")]
                    if members:
                        ids.extend((f, True) for f in members)
                        continue
                if not self._defined(cid):
                    self.defects.append(f"DANGLING: {cid} cited at {rel}:{line} is not in the index")
                else:
                    ids.append((cid, wide))
        return ids
    ...
    def _scan_sources(self):
        for sf in self.sources:
            base = (
                "sby"
                if sf.rel.endswith(".sby")
                else "rtl"
                if sf.rel.startswith("rtl/")
                else "sim"
                if sf.rel.startswith("sim/")
                else "fv"
            )
            stmts = [] if sf.rel.endswith((".sby", ".svh")) or sf.rel.startswith("sim/") else parse_statements(sf)
            ...
            for m in RUN_RE.finditer(sf.text):
                pos = m.start()
                kind = base + ("-comment" if sf.in_comment(pos) else "-code")
                owner = next((o for o in owners if o[0] <= pos < o[1]), None)
                if owner:
                    kind = owner[2]
                for cid, wide in self._expand(m.group(0), sf.rel, sf.line_of(pos)):
                    self.occurrences.setdefault(cid, []).append((sf.rel, sf.line_of(pos), kind, wide))

            for st in stmts:
                if st["kind"] != "assert":
                    continue
                self.assert_total += 1
                runs = [RUN_RE.findall(sf.text[s:e]) for s, e in st["owned"]]
                runs = [r for chunk in runs for r in chunk]
                runs.extend(registry_runs(sf, st))
                ids = [(cid, wide) for r in runs for cid, wide in self._expand(r, sf.rel, st["line"])]
                if not ids:
                    self.assert_unsourced.append((sf.rel, st["line"], st["label"]))
                else:
                    for cid, wide in ids:
                        self.assert_ev.setdefault(cid, []).append((sf.rel, st["line"], st["label"], wide))
    ...
    def classified(self):
        ...
        for cid in list(self.fact_lines) + list(self.clause_lines):
            if cid in self.assert_ev:
                ...
                uncited.append(cid)
        return (sorted(verified, key=id_key), sorted(referenced, key=id_key), sorted(uncited, key=id_key))

    def section_wide_only(self):
        """Verified ids whose every assert attribution is section-wide."""
        return sorted((cid for cid, evs in self.assert_ev.items() if all(e[3] for e in evs)), key=id_key)
    ...
    def report(self):
        ...
        out = [
            "== vibe-pio traceability audit ==",
            f"fact index   : {len(self.fact_lines)} facts from docs/pio-spec.md",
            (
            ...
            (
                ...
                f"rtl, "
                f"{sum(1 for s in self.sources if s.rel.startswith('formal/') and s.rel.endswith('.sv'))} formal sv, "
            ...
            f"citations    : {n_occ} occurrences, {len(self.occurrences)} distinct IDs cited",
            "",
            "-- hard defects --",
        ]
        out.extend(self.defects or ["(none)"])
        out.append("")
        out.append("-- asserts with no direct or registry-linked citation (review) --")
        if self.assert_unsourced:
            out.extend(f"  {rel}:{line}  {label or '(unlabeled)'}" for rel, line, label in self.assert_unsourced)
        else:
            out.append("  (none)")
        out.append("")
        out.append(
            f"-- verified by >=1 assertion: {len(verified)} of "
            f"{n_total} "
            f"({len(self.section_wide_only())} section-wide only) --"
        )
        out.append("")
        sim_only = sum(1 for cid in referenced if all(o[2].startswith("sim") for o in self.occurrences[cid]))
        out.append(f"-- referenced but never asserted ({len(referenced)}; {sim_only} cited only in sim/) --")
        for cid in referenced:
            rel, line, kind, _ = self.occurrences[cid][0]
            out.append(f"  {cid:<14} {kind:<13} {rel}:{line}")
        out.append("")
        out.append(f"-- never cited in rtl/, formal/ or sim/ ({len(uncited)}) --")
...
def self_test():
    ...
    try:
        ...
        (tmp / "formal" / "dut.sby").write_text("# config citing CC-1\n[tasks]\nbmc\n")

        def run_audit():
            return Audit(
                tmp / "spec.md",
                tmp / "cc.md",
                [str(tmp / "rtl" / "*.sv"), str(tmp / "formal" / "*.sv"), str(tmp / "formal" / "*.sby")],
                repo=tmp,
            )
        ...
        checks = [
            ("range-gap", {"SPEC-2-1", "SPEC-2-2", "SPEC-2-5"} <= set(aud.occurrences)),
            (
                "secrange",
                "SPEC-5-6" in aud.occurrences
            ...
            ("registry-link", "SPEC-8-1" in assert_ev_ids and "SPEC-8-2" in assert_ev_ids),
            ("registry-continuation", "SPEC-5-1" in assert_ev_ids),
            ("prop-range-preamble", any(e[2] == "a_p9" for e in aud.assert_ev.get("SPEC-2-2", []))),
            ("guard-climb", any(e[2] == "a_p10" for e in aud.assert_ev.get("CC-1", []))),
            ("unsourced-none", not aud.assert_unsourced),
            ("assume-not-verify", "SPEC-5-6" in aud.occurrences and "SPEC-5-6" not in assert_ev_ids),
            ("sby-cited", "CC-1" in aud.occurrences),
            ("no-dangling", not any(d.startswith("DANGLING") for d in aud.defects)),
            ("no-parse", not any(d.startswith("PARSE") for d in aud.defects)),
            ("no-index", not any(d.startswith("INDEX-MISMATCH") for d in aud.defects)),
        ]
        ...
        (tmp / "rtl" / "bad.sv").write_text("// dangling: SPEC-2-99\n// parse trap: CC-1..CC-x\n")
        aud3 = run_audit()
        (tmp / "rtl" / "bad.sv").unlink()
        mut = [
            ("mut-dangling", any("SPEC-2-99" in d and d.startswith("DANGLING") for d in aud3.defects)),
            ("mut-parse", any(d.startswith("PARSE") for d in aud3.defects)),
        ]

        (tmp / "spec.md").write_text("# spec\n## 2\n- [SPEC-2-1] a.\n- [SPEC-2-1] again.\n")
        (tmp / "cc.md").write_text("# cc\n**CC-1 [MODEL] (a).** body.\n## 11. Clause index\n| CC-9 | missing body |\n")
        aud4 = run_audit()
        mut.extend(
            [
                ("mut-dup-id", any(d.startswith("DUP-ID") for d in aud4.defects)),
                ("mut-index", any(d.startswith("INDEX-MISMATCH") for d in aud4.defects)),
            ]
        )
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="SPEC-/CC- traceability audit (see module docstring)")
    ap.add_argument("--spec", default="docs/pio-spec.md")
    ap.add_argument("--cc", default="docs/cycle-contract.md")
    ap.add_argument("--strict", action="store_true", help="also fail on unsourced assertions")
    ...
    aud = Audit(args.spec, args.cc, ["rtl/*.sv", "formal/*.sv", "formal/*.sby", "sim/*.sv", "sim/*.svh"])
    print(aud.report())
