...
const WRAP_TARGET = LEVEL.wrapTarget, WRAP_LAST = LEVEL.wrapLast;
const FIFODEPTH = LEVEL.fifoDepth, WIN = 128, BITCYC = LEVEL.bitCyc;
...
let V = {
  state: { cycle:0, pin:0, pc:0, displayPc:0, phase:"OFF", delay:0,
           x:0, y:0, osr:0, isr:0, osrCnt:0, isrCnt:0,
           txLevel:0, txEmpty:true, txFull:false, txWords:[],
           monitor:{decoded:"",frameOff:null},
           wave:{pins:[],tags:[],startCycle:0}, flashes:{}, refused:0 },
  ready:false, inflight:false, lastFlashClk:-1,
};
const engineUrl = new URL("../build/web/pio_engine.js", location.href).href;
worker.postMessage({cmd:"init", engineUrl});
worker.onmessage = (e)=>{
  const m = e.data;
  if (m.cmd === "werr") {          // worker-side failure surfaced to the view
    const f=document.querySelector("footer");
    if (f) f.innerHTML=`<span style="color:var(--red)">ENGINE WORKER ERROR: ${m.message||"unknown"}</span>`;
    return;
  }
  if (m.cmd === "ready") {
    V.ready = true;
...
worker.onerror = (ev)=>{
  const f=document.querySelector("footer");
  if (f) f.innerHTML=`<span style="color:var(--red)">ENGINE WORKER ERROR: ${ev.message||"load failed"}</span>`;
};

function post(cmd){ if (V.ready) worker.postMessage(cmd); }
...
function runTick(){                       // one clk per interval, pipelined
  if(V.inflight) return;                  // a cycle is still in flight
  V.inflight=true; post({cmd:"run",cycles:1});
}
function pause(){clearInterval(timer);timer=null;V.inflight=false;
  $("brun").textContent="▶ RUN";$("brun").classList.remove("on");}
function enableCtrls(on){
  for (const id of ["breset","brun","bstep","binsn"]) $(id).disabled=!on;
}
$("brun").onclick=run;
$("breset").onclick=()=>{pause();post({cmd:"reset"});};
$("bstep").onclick=()=>{pause();post({cmd:"step"});};
$("binsn").onclick=()=>{pause();post({cmd:"stepInsn"});};
$("speed").onchange=()=>{if(timer){clearInterval(timer);timer=setInterval(runTick,+$("speed").value);}};
...
function refuseFlash(){
  const f=$("fifometa");f.classList.remove("fifofull");void f.offsetWidth;f.classList.add("fifofull");
}

document.addEventListener("keydown",e=>{
  if(e.key==="Escape"&&!POP.hidden){popupHide();HOST.classList.remove("picking");return;}
  const t=e.target;
  if(/^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName)||t.isContentEditable)return;
  if(!V.ready)return;
  if(e.code==="Space"){e.preventDefault();pause();post({cmd:"step"});}
...
function updateUnbuilt(){
  ...
  const u=$("unbuilt"); if(u) u.hidden=!diverged;
}
function renderAlloc(){
  ...
  info.innerHTML=`side ${ALLOC.sideBits}b${ALLOC.opt?"+opt":""} · delay [0..${maxDelay()}]`+
    (redecoded?` <span style="color:var(--red)" title="same stored bits, re-decoded under this allocation — the machine runs what the bits now say (and the engine really does: PINCTRL/EXECCTRL were re-written)">· garbled</span>`:"");
  const st=$("sidetag");
  ...
  for(let i=0;i<32;i++){
    ...
    if(cur){
      if(st.phase==="DELAY"){
        ...
        c.textContent="delay ⟳"+st.delay; chips.appendChild(c);
...
function renderExec(st){
  const disp=st.displayPc, p=PROG[disp], e=EFF[disp]||{delay:0,side:null};
  $("execins").innerHTML = p
  ...
  if(st.phase==="EXEC"){n.textContent="EXEC";n.className="st";fill.style.width="0%";
    sub.textContent="instruction runs this cycle";}
  else if(st.phase==="DELAY"){n.textContent="DELAY "+st.delay;n.className="dl";
    fill.style.width=(100*(1-st.delay/(e.delay||1)))+"%";
    sub.textContent=st.delay+" cycles until PC → "+st.pc.toString().padStart(2,"0");}
  else if(st.phase==="STALL"){n.textContent="STALL";n.className="wa";fill.style.width="0%";
    sub.textContent="pull block · TX empty · line held 1 (side)";}
  else {n.textContent="—";n.className="";fill.style.width="0%";
...
function renderWave(st){
  ...
  const pins=w.pins.slice(-n), tags=w.tags.slice(-n);
  const yOf=p=>p?HI:LO;
  let i=0;
  while(i<n){
    let j=i+1;
    ...
    if(i>0)s+=`<path d="M${x0} ${yOf(pins[i-1])} L${x0} ${y}" stroke="${col}" stroke-width="2"/>`;
...
function render(st){
  renderProgram(st); renderExec(st); renderFrameMap(st); renderRegs(st);
  renderWave(st); renderMonitor(st);
  const fl=st.flashes||{};
  if(st.cycle!==V.lastFlashClk){          // one flash per clk, not per message
...
function popupHide(){POP.hidden=true;}
function accept(c){
  if(c.kind==="pick"){
...
enableCtrls(false);
buildProgram(); buildBits($("osrbits")); buildBits($("isrbits")); render(V.state);
